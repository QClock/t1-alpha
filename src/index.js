...
if (config.TESSEL && config.TESSEL_VERSION == 2) {
    config.USE_ADMIN = true;
...
function getTime () {
    var t = new Date((+new Date() + config.NTP_OFFSET) + config.HW_LAG);

    process.binding('tm').timestamp_update(t.getTime() * 1000); // conversion
